class ObjectivesController < ApplicationController
  ...
  # POST /objectives.xml
  def create
    params[:objective][:project_id] = params[:project_id]
    @objective = Objective.new(params[:objective])

    respond_to do |format|
      if @objective.save
        format.html { redirect_to(new_objective_task_path(@objective), :notice => 'Objective was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @objective.update_attributes(params[:objective])
        format.html { redirect_to([@project, @objective], :notice => 'Objective was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    @objective = @project.objectives.find(params[:id])
    @objective.destroy
